<h1 align="center">Eduardo Olivares Hernández</h1>

<p align="center">
  <b>SOC Analyst · Blue Team · AI-driven threat detection</b><br>
  <sub>📍 Badalona · Barcelona</sub>
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/eduolihez/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="https://eduolihez.com"><img src="https://img.shields.io/badge/Portfolio-10B981?style=for-the-badge&logo=astro&logoColor=white" alt="Portfolio" /></a>
  <a href="https://www.credly.com/users/eduolihez"><img src="https://img.shields.io/badge/Credly-FF6B00?style=for-the-badge&logo=credly&logoColor=white" alt="Credly" /></a>
  <a href="mailto:eduardo@eduolihez.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>
</p>

<p align="center">
  <img src="https://img.shields.io/github/followers/eduolihez?style=flat-square&logo=github&color=10B981&labelColor=181717" alt="GitHub followers" />
</p>

<p align="center">
  <a href="readme.md">Versión en Español</a> · <b>English version</b>
</p>

---

### 👋 About Me
Security analyst specialized in threat detection, incident response and automation with Python. I work daily with XDR/SIEM (Trend Micro Vision One), Fortinet (FortiGate/FortiAnalyzer) and Active Directory, applying AI to threat investigation and phishing analysis. Certified by Fortinet (NSE), Microsoft and Trend Micro. Based in Badalona (Barcelona).

---

### 📊 GitHub Activity
<p align="center">
  <img height="165" src="https://eduolihez.com/api/github-stats.php" alt="GitHub stats" />
  <img height="165" src="https://eduolihez.com/api/github-langs.php" alt="Most used languages" />
</p>
<p align="center">
  <img height="165" src="https://eduolihez.com/api/github-streak.php" alt="Contribution streak" />
</p>

---

### 💻 Open Source

<details>
<summary><b>🛡️ Security and threat intelligence</b></summary>
<br>

| Project | Description | Stack |
|---|---|---|
| 🛰️ **[KEV Digest](https://github.com/eduolihez/kev-digest)** | Watches the CISA KEV catalog daily: flags confirmed actively-exploited vulnerabilities, marks the ones used in ransomware, and keeps a versioned record. No dependencies, runs on GitHub Actions. | Python · GitHub Actions |
| 📊 **[Vision One CREM Report Generator](https://github.com/eduolihez/vision-one-crem-report-generator)** | Generates risk and exposure reports from Trend Vision One, enriched with NVD, KEV and EPSS data, as interactive HTML, Word and PDF. | Python · pandas · Flask |
| 🛡️ **[Blue Team Hub](https://github.com/eduolihez/eduolihez.github.io)** | Client-side cybersecurity tools + [KEV Watch](https://eduolihez.github.io/tools/kev-watch), daily automated monitoring (GitHub Actions) of the CISA KEV catalog. | Astro · TailwindCSS |
| 🔐 **[Password Centinel](https://github.com/eduolihez/password-centinel)** | 100% local password manager: AES-256-GCM, TOTP/2FA, HIBP check via k-anonymity. | JavaScript · WebCrypto |
| 🦊 **[NorthGate Browser](https://github.com/eduolihez/northgate-browser)** | Mullvad/Firefox fork with on-device phishing detection via ONNX/Rust, no telemetry. | Rust · ONNX · JavaScript |

</details>

<details>
<summary><b>🛠️ Tools and applications</b></summary>
<br>

| Project | Description | Stack |
|---|---|---|
| 🌐 **[eduolihez.com](https://github.com/eduolihez/eduolihez.com)** | Trilingual (ES/EN/CA) portfolio and blog — static Astro + a custom PHP/MySQL API. | Astro · TypeScript · PHP |
| 🔑 **[BinCat](https://github.com/eduolihez/BinCat)** | Flask SDK and dashboard to generate, validate and revoke Fernet/JWT tokens securely. | Python · Flask · Cryptography |
| 🌊 **[Dewi App](https://github.com/eduolihez/hackathon-Dewi)** | Winning prototype of the 8th TecnoCampus Hackathon for real-time water consumption monitoring. | Next.js · TypeScript |

</details>

<details>
<summary><b>📦 Closed-source projects</b></summary>
<br>

* 🤖 **[PromptMaster](https://eduolihez.com/projects/promptmaster/)** — Structured prompt-engineering extension for ChatGPT, Claude and Gemini.

</details>

---

### 🎓 Featured Certifications

<details>
<summary><b>🔎 View verified badges</b></summary>
<br>

| Badge | Certification |
|:---:|---|
| <a href="https://www.credly.com/badges/d211824c-9076-4d47-a3e4-df212f541969"><img src="https://images.credly.com/size/110x110/images/20082fc1-94af-4773-9df0-28856b566748/image.png" width="64" alt="Fortinet Certified Associate Cybersecurity" /></a> | 🛡️ **[Fortinet Certified Associate Cybersecurity](https://www.credly.com/badges/d211824c-9076-4d47-a3e4-df212f541969)** — Associate-level network security certification. |
| <a href="https://www.credly.com/badges/9a8e25b3-ca1d-4010-ba2e-59378b3eb668"><img src="https://images.credly.com/size/110x110/images/7ca06297-1543-4ed2-b9fc-7f0af8635700/blob" width="64" alt="Fortinet NSE 3 Certified in Cybersecurity" /></a> | 🛡️ **[Fortinet NSE 3 Certified in Cybersecurity](https://www.credly.com/badges/9a8e25b3-ca1d-4010-ba2e-59378b3eb668)** — Network Security Expert, level 3. |
| <a href="https://www.credly.com/badges/3010ded8-d149-438c-b041-0da42fc58d09"><img src="https://images.credly.com/size/110x110/images/4136ced8-75d5-4afb-8677-40b6236e2672/azure-ai-fundamentals-600x600.png" width="64" alt="Microsoft Certified: Azure AI Fundamentals" /></a> | ☁️ **[Microsoft Certified: Azure AI Fundamentals](https://www.credly.com/badges/3010ded8-d149-438c-b041-0da42fc58d09)** — Cloud infrastructure and machine learning applied to security. |
| <a href="https://www.linkedin.com/in/eduolihez/details/certifications/"><img src="https://img.shields.io/badge/Trend_Micro-D71920?style=flat-square&logoColor=white" width="120" alt="Trend Micro Vision One" /></a> | 🔒 **[Trend Micro Vision One](https://www.linkedin.com/in/eduolihez/details/certifications/)** — SecOps, AI Security, Threat Intelligence and Cloud Security. |

📜 Full, verifiable list on **[Credly](https://www.credly.com/users/eduolihez)**.

</details>

---

### 📬 Connect
* 💼 Professional background on **[LinkedIn](https://www.linkedin.com/in/eduolihez/)**.
* 🌐 Portfolio, blog and write-ups at **[eduolihez.com](https://eduolihez.com)**.
* ✉️ Direct contact: **[eduardo@eduolihez.com](mailto:eduardo@eduolihez.com)**.
