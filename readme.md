<div align="center">
  
# 🛡️ Eduardo Olivares - Tech Lead & Cybersecurity Enthusiast

### **Técnico de Sistemas | Especialista en SysAdmin y Desarrollo con Python**
<p>📍 Badalona, Barcelona, España</p>

</div>

---

## 🚀 Perfil Profesional

Soy un **Técnico de Sistemas** con experiencia probada en **Soporte IT Nivel 1 y 2** y un enfoque sólido en **SysAdmin** y **Ciberseguridad**. Mi pasión se centra en la **automatización de procesos** con **Python** y en la creación de soluciones eficientes que impactan directamente en la continuidad operativa.

> **💡 Logros Destacados:**
> * Reducción de **más del 15%** en los tiempos de respuesta de tickets (SLA) mediante scripting.
> * Liderazgo y coordinación de equipos, **mejorando la eficiencia en un 25%**.
> * Enfoque actual en **SecOps** y la aplicación de ciberseguridad en el ciclo de desarrollo.

---

## 🛠️ Stack Tecnológico <img src="https://img.shields.io/badge/-FullStack-22B2DA?style=for-the-badge&logo=codeigniter&logoColor=white" />

Mi experiencia abarca desde la administración de infraestructura hasta el desarrollo full-stack:

<div align="center">
  <img src="https://skillicons.dev/icons?i=py,js,html,css,php,mysql,git,linux,windows,bash,raspberrypi,arduino" height="50" alt="Tech Stack Icons"  />
</div>

<details>
  <summary><strong>Ver Detalle por Área</strong></summary>
  
| Área | Tecnologías Clave |
| :--- | :--- |
| **Desarrollo** | Python, JavaScript, HTML5, CSS3, PHP, MySQL |
| **Sistemas** | Linux Server, Windows Server (Active Directory), Bash Scripting |
| **IoT & Hardware** | Raspberry Pi, Arduino (Robótica y Sistemas Embebidos) |
| **Control de Versiones** | Git / GitHub |
  
</details>

---

## ⚙️ Metodologías y Herramientas IT **(¡NUEVO!)**

Experiencia trabajando con prácticas modernas de gestión y seguridad:

* **Gestión de Infraestructura:** Active Directory (Windows Server), Virtualización, Administración de redes.
* **Seguridad:** Hardening de sistemas, Ethical Hacking (TryHackMe), SecOps.
* **Metodología:** Gestión de Tickets (SLA), Documentación, Automatización de tareas.
* **Herramientas:** Shell Scripting (Bash/PowerShell), Formspree (Contacto Web), Leaflet (Geolocalización).

---

<div align="center">
  <h2>✨ Proyectos Destacados</h2>
</div>

| Proyecto | Descripción | Tecnologías | Ver |
| :--- | :--- | :--- | :--- |
| **Passwd Centinel** | Extensión de Chrome para **gestión segura de contraseñas** y auditoría de seguridad en tiempo real. | `JS` `Chrome Ext` `Seguridad` | [![Ver Proyecto](https://img.shields.io/badge/-Web-1D9BF0?style=flat-square&logo=chrome&logoColor=white)](https://passwdcentinel.eduolihez.com) |
| **Més Badalona** | Plataforma de participación ciudadana geolocalizada para **reportar incidencias locales**. | `PHP` `MySQL` `Web` | [![Ver Proyecto](https://img.shields.io/badge/-Web-0077B5?style=flat-square&logo=openstreetmap&logoColor=white)](https://mesbadalona.eduolihez.com) |
| **Guardianes Digitales** | **Curso interactivo** gamificado de concienciación sobre ciberseguridad. | `Ciberseguridad` `Educación` `JS` | [![Ver Curso](https://img.shields.io/badge/-Curso-E4405F?style=flat-square&logo=coursera&logoColor=white)](https://eduolihez.com/guardianes-digitales/index.html) |
| **FollowGuard** | Herramienta de análisis para Instagram. Detecta unfollowers y gestiona tu comunidad. | `API` `Analytics` `Seguridad` | [![Ver Proyecto](https://img.shields.io/badge/-Web-787878?style=flat-square&logo=instagram&logoColor=white)](https://followguard.eduolihez.com) |

---

## 🏆 Certificaciones & Progreso

<div align="center">
  
| Certificación | Área | Credencial |
| :--- | :--- | :--- |
| **IT Specialist** | Python Programming | <a href="https://www.credly.com/badges/6ea9eebb-bf6b-4369-82ee-845195fc8652/public_url" target="_blank"><img src="https://img.shields.io/badge/-Ver%20Credencial-0077B5?style=for-the-badge&logo=credly&logoColor=white" alt="Credly Badge" /></a> |
| **IC3 Digital Literacy** | Competencias Digitales | <a href="https://www.credly.com/badges/cc7885f6-f69b-473a-ab89-e3a07f7a49a3/public_url" target="_blank"><img src="https://img.shields.io/badge/-Ver%20Credencial-0077B5?style=for-the-badge&logo=credly&logoColor=white" alt="Credly Badge" /></a> |
| **B2 First Certificate** | Inglés (Cambridge) | <a href="src/First_Certificate.jpg" target="_blank"><img src="https://img.shields.io/badge/-Ver%20Certificado-28A745?style=for-the-badge&logo=google-drive&logoColor=white" alt="Google Drive Link" /></a> |

</div>

---

<div align="center">

## 📊 Actividad en GitHub

<img src="https://streak-stats.demolab.com?user=eduolihez&locale=es&mode=daily&theme=dracula&hide_border=false&border_radius=5&order=3" height="160" alt="GitHub Streak"  />
<img src="https://github-profile-trophy.vercel.app?username=eduolihez&theme=dracula&column=-1&row=1&margin-w=8&margin-h=8&no-bg=false&no-frame=false&order=4" height="160" alt="Trofeos de GitHub"  />

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/eduolihez/eduolihez/output/pacman-contribution-graph-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/eduolihez/eduolihez/output/pacman-contribution-graph.svg">
  <img alt="Gráfico de Contribuciones de Pac-Man" src="https://raw.githubusercontent.com/eduolihez/eduolihez/output/pacman-contribution-graph.svg">
</picture>

---

## 📧 Contacto Rápido **(¡NUEVO!)**

Si buscas un profesional de sistemas con mentalidad de desarrollador, hablemos.

<a href="https://www.linkedin.com/in/eduolihez/" target="_blank">
  <img src="https://img.shields.io/badge/Conectar%20en%20LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>
<a href="mailto:eduolihez@gmail.com">
  <img src="https://img.shields.io/badge/Enviar%20Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
</a>
<a href="src/Edu_Olivares_CV.pdf" download="Eduardo_Olivares_CV.pdf">
  <img src="https://img.shields.io/badge/Descargar%20CV-0077B5?style=for-the-badge&logo=adobeacrobatreader&logoColor=white" alt="Descargar Curriculum" />
</a>
<a href="https://tryhackme.com/p/eduolihez" target="_blank">
  <img src="https://img.shields.io/badge/TryHackMe-88cc14?style=for-the-badge&logo=tryhackme&logoColor=white" alt="TryHackMe Profile" />
</a>

</div>
